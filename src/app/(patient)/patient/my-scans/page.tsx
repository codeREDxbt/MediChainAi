...
export default function MyScansPage() {
  ...
  const getDisplayConfidence = (scan: Scan): number | null =>
    getActualModelConfidence({
      findings: scan.findings,
      fallbackConfidence: typeof scan.aiScore === "number" && scan.aiScore > 0 ? scan.aiScore : null,
    });
                  ...
                  {filteredScans.length === 0 ? (
                    ...
                    >
                      {filteredScans.map((scan) => {
                        ...
                        const displayConfidence = getDisplayConfidence(scan);
                                          ...
                                          <div className="flex flex-col items-end">
                                            <div className="flex items-center gap-1.5 mb-1 bg-white/5 px-2 py-1 rounded-lg">
                                              ...
                                              <span className="text-lg font-bold text-emerald-400 leading-none">
                                                {displayConfidence !== null ? `${formatConfidence(displayConfidence)}%` : "N/A"}
                                              </span>
                                            </div>
                                            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-medium">
                                              {displayConfidence !== null ? "Confidence" : "No Actual Confidence"}
                                            </span>
                                          </div>
                                        ) : scan.status === "Processing" ? (
