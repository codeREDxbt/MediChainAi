    ...
    if (monaiUrl) {
        try {
            ...
            if (res.ok) {
                ...
                results.monai = { status: "online", ...data };
            } else {
    ...
    return NextResponse.json(
        {
            ...
            providers: results,
            capabilities: {
                "Image segmentation (CT/MRI/DICOM/NIfTI)": results.monai
                    ? (results.monai as { status: string }).status === "online"
                    : false,
                "Scan description / radiology text (2D images)": !!openRouterKey,
